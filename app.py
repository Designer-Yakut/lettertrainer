...
#  ЗАПУСК
# ------------------------------------------------
#if __name__ == "__main__":                          #  Необходимо закоментить при запуске на Render, ожидает app как Flask-приложение, а ты запускаешь как скрипт
    """
    Запуск Flask-приложения локально.
    Хост 0.0.0.0 — доступ с других устройств в сети.
    """
    #app.run(host="0.0.0.0", port=5000, debug=True) #  Необходимо закоментить при запуске на Render, ожидает app как Flask-приложение, а ты запускаешь как скрипт
